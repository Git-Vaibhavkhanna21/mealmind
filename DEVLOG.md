...
The new `bottom-nav.tsx` doesn't include a Sign Out control, and the old `nav.tsx` it replaces — which did — was deleted, per the exact component spec given (three tabs only). This removes the only Sign Out entry point currently in the UI; flagged rather than silently addressed, since it wasn't part of this PR's scope and a reasonable home for it (e.g. the still-placeholder `/settings` page) is a call for a follow-up PR, not this one.
