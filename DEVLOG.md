...
## PR #13 — Fix Railway nixpacks config
`api/nixpacks.toml` and `api/railway.json` added to `api/` to resolve Railway's failure to detect the start command. Root cause: Railway's Nixpacks builder was running from the repo root, where its own auto-detection (based on the root-level `requirements.txt`) conflicted with PR #12's `railway.json`, which set a custom `buildCommand` pointing at `api/requirements.txt` while still expecting `uvicorn api.main:app` from the repo root. This PR's config assumes the opposite topology — Railway's Root Directory set to `api/` in the dashboard, `api/nixpacks.toml` with a start-command-only build (letting Nixpacks auto-detect `api/requirements.txt` once it's actually building from within `api/`), and `api/railway.json`'s start command changed to `uvicorn main:app` (not `api.main:app`, since `api/` is now the working directory itself). That's the inverse of PR #12's deliberate repo-root-rooted setup, and on its own this PR ships a service that can't yet resolve its own sibling imports (`agents/`, `workflows/`, `mcp_servers/` still lived at the repo root at this point) — called out explicitly as follow-up work, and resolved two PRs later once PR #14 physically moved those packages inside `api/` to match.
...
## PR #20 — Design system, bottom nav, landing page, and onboarding redesign
### What was built
A full visual redesign covering the app shell and two full pages: a new CSS custom-property design token system (`app/globals.css`) — warm cream/amber palette, Lora for display headlines, DM Sans for body/UI text — wired into Tailwind v4 via `@theme inline` so tokens like `--amber`/`--bg`/`--text-mid` generate real utility classes (`bg-amber`, `text-text-mid`, etc.) alongside a `font-display` utility for Lora. `layout.tsx` loads both fonts via `next/font/google`, sets the body background, and replaces the old top nav (`components/nav.tsx`, deleted) with a new `components/bottom-nav.tsx` — a fixed, 72px-tall, three-tab bottom bar (Pantry/Recipes/Shopping List, Lucide icons) that highlights the active tab with an amber pill and hides itself on `/` and `/onboarding`. The landing page (`app/page.tsx`) and its `GoogleSignInButton` were rebuilt to the exact spec (Lora wordmark, tagline, supporting line, full-width amber CTA). `app/onboarding/page.tsx` and `components/onboarding-client.tsx` became a three-step wizard (cooking skill cards with icons, dietary-restriction pills, a large-tap-target cooking-time grid), with progress dots, a back chevron, and a single amber Next/Get Started button that submits on the final step.

Added before merging: a fourth `Settings` tab on `bottom-nav.tsx` (Lucide `Settings` icon) pointing to a new, minimal `app/settings/page.tsx` — the user's email and a `components/sign-out-button.tsx` (amber, matching the rest of the design system), nothing else. `/settings` is protected the same way every other authenticated page in this app already is — a server-side `supabase.auth.getUser()` check that redirects to `/` if there's no session — since there's no centralized middleware-based route list to update; each page owns its own guard.

### Architectural decisions
Verified the Tailwind v4 token wiring by actually fetching and inspecting the compiled CSS output from a running `next dev` server, rather than trusting that `@theme inline` mapping an arbitrary custom-property name (`--font-display`, not one of Tailwind's built-in `sans`/`serif`/`mono` keys) would generate a real utility class — confirmed `.font-display { font-family: var(--font-lora); }` and every color utility (`bg-amber`, `bg-bg`, `text-muted`, `text-text-mid`, etc.) were all present and correctly wired before treating the design system as done. Colors are kept as plain `:root` custom properties (matching the exact token block given) with a separate `@theme inline` layer mapping names for utility generation; `--radius`/`--radius-sm` were deliberately left out of that mapping and used via Tailwind's arbitrary-value syntax (`rounded-[var(--radius)]`) instead, since Tailwind v4's built-in radius scale has its own key conventions and wiring a bare `--radius` into it risked ambiguity that plain custom-property references sidestep entirely.

The new `bottom-nav.tsx` initially shipped without a Sign Out control, and the old `nav.tsx` it replaced — which had one — was deleted, per the exact three-tab component spec given. That removed the only Sign Out entry point in the UI; flagged rather than silently fixed at the time, since it wasn't part of that PR's stated scope. Addressed before merge with the `/settings` addition above, once asked for explicitly.
...
## PR #22 — Add architecture diagrams to README
### What was built
Two Mermaid diagrams added to `README.md` after the Overview section: a system-architecture flowchart (Browser → Vercel Next.js → Railway FastAPI, branching to the Anthropic API, OpenAI API, and Supabase, plus TheMealDB feeding Supabase through the Recipe MCP Server and Google OAuth feeding Supabase Auth) and a sequence diagram for the receipt-to-pantry flow (User → Frontend → FastAPI → Receipt Parsing Workflow → Haiku → Expiration Workflow → Haiku/Sonnet subagent → Pantry MCP Server → Supabase → back to Frontend). Every edge/message is labeled with what actually flows over it — the internal API key header, `embeddings.create` calls, SQL, OAuth token exchange.

### Architectural decisions
Every labeled connection was cross-checked against the real source before being written into the diagram, rather than described from memory of the architecture — the internal API key header against `frontend/lib/python-api.ts` and `api/main.py`'s `_require_internal_api_key`, the OpenAI embeddings call against `api/agents/meal_recommender.py`, and the Google OAuth wiring against `google-sign-in-button.tsx`. Both diagrams were also extracted and rendered through `@mermaid-js/mermaid-cli` before committing, to catch any syntax errors that would otherwise only surface as a broken diagram on GitHub.

### Bugs found and fixed
None — this PR's own DEVLOG entry was originally omitted at the time it merged, caught during a later repo audit and backfilled in PR #23 (this omission is itself documented there, not silently corrected).

## PR #23 — Repo cleanup: fix stale docs, missing DEVLOG entries, and gitignore gaps
### What was built
A full repo audit (checking for secret leaks in git history, stale/orphaned files, and cross-file inconsistencies between README/DEPLOYMENT.md/DEVLOG.md and the actual code) surfaced six issues, fixed here: two missing DEVLOG entries backfilled (PR #13's nixpacks fix, PR #22's architecture diagrams — see both entries above); README's "Getting Started" placeholder, unchanged since before Phase 1 was built, replaced with real setup steps (env file copies, `pip install`/`uvicorn` for the backend, `npm install`/`npm run dev` for the frontend); a malformed live-URL Markdown link (`[(url)]`, rendering as literal text instead of a hyperlink) fixed in README, and DEPLOYMENT.md's still-unfilled `[LIVE_URL]` placeholder filled in to match; `frontend/README.md` — untouched `create-next-app` boilerplate referencing the Geist font (the app has used Lora/DM Sans since PR #20) and generic Next.js links — replaced with a one-line pointer to the real docs; root `.gitignore` gained `.DS_Store`/`.Rhistory`/`database/.DS_Store` entries (previously only `frontend/.gitignore` ignored `.DS_Store`, so these files surfaced as untracked noise in every `git status` at the repo root, despite never actually being committed); root `.env.example` gained a comment block explaining what it's for and that it's separate from `frontend/`'s own example files, since no doc anywhere referenced it despite every Python entry point (`api/`, `scripts/`, `database/seed_recipes.py`) loading a root `.env` via `load_dotenv()`.

### Architectural decisions
The audit confirmed no secrets were ever committed — `.env`, `.mcp.json`, and `frontend/.env.local` were checked against both current tracking (`git ls-files`) and full history (`git log --all`) and are clean in both — so this PR is purely documentation and gitignore hygiene, no credential rotation needed. The duplicate root/`api/` `requirements.txt` and the pre-design-system styling still in `recipes-client.tsx`/`shopping-list-client.tsx` were both surfaced by the same audit but deliberately left alone here — the former isn't a bug (root's copy serves local script/seed usage, `api/`'s serves the deployed service), and the latter is explicitly scoped to a future design PR, not this cleanup.
...
## PR #25 — Eval harness PR 1: fixtures, rubrics, and eval log
### What was built
The `eval/` directory: fixed-answer fixtures for the three deterministic components — `fixtures/receipt_parsing.json` (15 cases), `fixtures/expiration.json` (26 cases, covering both confident Haiku estimates and the three Sonnet-escalation cases from PR #24 — kimchi, bone broth, rendered duck fat), `fixtures/deduction.json` (14 cases, ids 1–11/13–15, covering exact matches, plural/singular normalization, UK/US terminology and spelling variants, regional name variants, and deliberate no-match cases) — plus rubric-scored fixtures for the two judgment-based components, `fixtures/recommendation.json` (10 cases, including one deliberate repeat of case 1 as a same-input consistency check) and `fixtures/shopping_list.json` (5 cases). `rubrics/recommendation.txt` and `rubrics/shopping_list.txt` define a 0/1/2 scoring rubric per criterion (six criteria for recommendations, five for shopping lists) with a JSON output contract for an LLM-as-judge to fill in. `EVAL_LOG.md` documents three manual Observation → Measurement → Diagnosis → Fix → Re-measurement testing cycles from before this harness existed (the ivfflat empty-bucket bug from PR #6, the Sonnet prose-prefixed-JSON bug also from PR #6, and PR #24's expiry_source/expiry_confidence addition). `README.md` explains the two eval loops (manual log vs. this harness), how 0–2 rubric scores normalize to a 0–100 report, the 70/100 pass threshold per component, and the baseline/regression flow. No harness code yet — every file here is JSON, text, or Markdown; the Python runner that actually executes fixtures against the live agents lands in PR 2.

### Architectural decisions
Deterministic components (parsing, expiration, deduction) get fixed expected-value fixtures because there's a single correct answer to check against. The two components with genuinely open-ended output — meal recommendation and shopping list generation, where there's no single "correct" ranked list — get rubric-scored fixtures instead, judged 0/1/2 per criterion by a future LLM-as-judge call rather than exact-match, since forcing a fixed expected output onto a judgment call would just be testing whether the model matches one arbitrary example rather than whether its reasoning is actually good.

The task specified this PR's fixture case data in two batches. The first batch (receipt parsing, expiration, both rubrics, the log, the readme) was fully specified inline. The second batch's initial request referenced "the fixture plan above" for `recommendation.json`/`deduction.json`/`shopping_list.json`'s case data — but no such plan was actually included in that message, and a repo search turned up nothing matching either. Flagged this rather than inventing plausible-looking pantry contents, recipe names, and expected-match values under the label "exactly as specified" — fabricated fixture data with fabricated provenance would have been worse than an incomplete PR, since a fixture suite silently full of made-up cases undermines the entire point of having fixtures. Built everything that was actually specified, held off on the three ambiguous files, and asked for the real case data before proceeding; it was then supplied in full and transcribed exactly as given.

### Bugs found and fixed
None — every fixture file was validated as parseable JSON (`json.load`) and case counts were checked against the specified totals (15/26/14/10/5, with `deduction.json`'s ids correctly skipping 12) before committing.

## PR #26 — Eval harness PR 2: runner, judge, scoring, and report
### What was built
`eval/judge.py`: a single `judge_output(component, input_data, output_data, rubric_text)` function — Sonnet reads a rubric plus the input/output pair, returns the `{"scores": {...}, "reasoning": {...}}` JSON contract the rubric specifies. `eval/harness.py`: five evaluators, one per PR 1 fixture file, run by `main()` and written to `eval/results/run_{timestamp}.json` and `eval/results/latest.json`. Three are deterministic (`eval_receipt_parsing` — fuzzy name match via `difflib.SequenceMatcher` plus exact quantity/unit match, scored by precision/recall; `eval_expiration` — checks the estimated date falls in `[min_days, max_days]` plus `expiry_source`/`expiry_confidence` match; `eval_deduction` — checks whether the agent's plan matched an ingredient at all). Two are rubric-scored via `judge.py` (`eval_recommendation`, `eval_shopping_list`), each producing a weighted 0–100 score per case. `eval/report.py` prints the pass-rate/score table and a breakdown of the lowest-scoring component's failures. `eval/regression.py` diffs `baseline.json` against `latest.json` and flags any component whose pass rate drops >5 points or average score drops >5 points; deliberately doesn't create `baseline.json` itself (per the task) and exits 1 with a clear message if it's missing, rather than silently no-opping.

Ran `python3 eval/harness.py` against the live Anthropic/OpenAI/Supabase APIs as the first real run before committing, per the task's requirement. All 70 fixture cases executed with **zero exceptions** (`grep`-verified: no `"error"` key anywhere in `latest.json`). Results: Receipt Parsing 2/15 (13%), Expiration 13/26 (50%), Recommendation 10/10 (89/100 avg), Deduction 11/14 (79%), Shopping List 4/5 (80/100 avg) — 3/5 components above the 70/100 threshold.

### Architectural decisions
Two places where the harness deliberately doesn't follow a literal reading of the task, because the literal reading can't work against the code that actually exists (checked against source, not guessed):

1. **Expiration eval calls the workflow, not the agent.** The task named `api/agents/expiration.py`'s `estimate_batch` as what to call, but that function has no `expiry_source`/`expiry_confidence` fields and never calls the Sonnet subagent — those only exist on `api/workflows/expiration_workflow.py`'s `estimate_expirations` (added in PR #24 for exactly this purpose). Calling `estimate_batch` alone would make the three Sonnet-escalation fixtures (kimchi, bone broth, rendered duck fat) permanently unpassable, since the field the fixture checks would never exist. Called the workflow function instead, one single-item list per case, which still matches "single item" and additionally makes the escalation path actually testable.
2. **Deduction eval needs Supabase credentials, not just `ANTHROPIC_API_KEY`.** `api/agents/pantry_deductor.py` has no standalone "match one ingredient against a mock pantry" function — its only real function, `build_deduction_plan(recipe_id, user_id)`, always reads a recipe and a pantry from Supabase by id. To exercise it per-fixture with a controlled pantry (as the task asked — "the real match function with a mock pantry"), the harness seeds a throwaway single-ingredient recipe row and a scoped pantry per case in Supabase, then deletes the recipe row afterward (cascades to any history row via the FK's `on delete cascade`).

A dedicated `EVAL_USER_ID` (deterministic via `uuid.uuid5`, not the `TEST_USER_ID` scripts/test_*.py use) keeps the harness's per-case seed/reset cycles from ever clobbering state those manual smoke tests depend on.

The 70/100 pass threshold from `eval/README.md` is applied uniformly across all five components, not just the two rubric-scored ones: for the three deterministic components, `pass_rate * 100` is compared against the same 70 threshold, so "component passing" means one consistent thing everywhere instead of two different concepts (a rate vs. a score) that happen to share a report table.

The first real run's low Receipt Parsing (13%) and Expiration (50%) pass rates are genuine findings, not harness bugs — verified this before writing anything here. `api/agents/parser.py`'s own extraction prompt lists `"each"` as an example unit for count items and never asks for metric conversion, so Haiku reliably returns `unit: "each"` where fixtures expect `null`, and `"0.5 gallon"` where fixtures expect a converted `"1.89 L"` — the fixtures encode a stricter contract than the current prompt actually implements. Similarly, three `deduction.json` no-match cases (natural vs. Greek yogurt, spring onions vs. red onion, parmesan vs. cheddar) came back matched by Haiku, just at low confidence (0.72, 0.35, 0.3) — `build_deduction_plan` has no confidence floor of its own; the frontend's amber/green confidence badge (PR #9) is the only place low-confidence matches get flagged, not filtered. None of this was "fixed" here — fixing prompts to chase eval scores is exactly the kind of scope creep this PR isn't meant to do; these results are the harness doing its job on its very first run, and now exist as a documented, reproducible starting point for whichever future PR addresses them.

### Bugs found and fixed
None in the harness itself — every one of the 70 cases in the first live run completed without an exception, confirmed by grepping `latest.json` for the `"error"` key every per-case exception handler would have written. The low scores described above are real model/prompt behavior, not harness defects.
