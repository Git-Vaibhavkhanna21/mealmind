...
## PR #28 — Fix: add confidence floor to pantry deduction to prevent false positive matches
### What was built
`api/agents/pantry_deductor.py`'s `build_deduction_plan` gained a code-level confidence floor (`CONFIDENCE_FLOOR = 0.75`): any Haiku match below that threshold is no longer trusted as-is — `pantry_item_id` is zeroed to `None`, `quantity_to_deduct` to `0`, and a new `match_found: False` field is set, while the entry itself stays in the returned list (with its original `pantry_item_name`/`unit`/`confidence`) rather than being dropped, so the PM-facing review UI can still show the user which recipe ingredients had nothing plausible in their pantry. Matches at or above the floor get `match_found: True` alongside their existing fields. The Haiku prompt itself also gained three explicit lookalike-pair rules: spring onions/scallions/green onions are not red/white/yellow onion, natural yogurt is not Greek yogurt, and crème fraîche is not sour cream or yogurt — all three should return no-match. `apply_deduction` now skips any plan entry with `match_found: False` before looking up its (now-`None`) `pantry_item_id`, since a caller that applies an unfiltered plan (rather than only the entries the user confirmed) would otherwise crash on `entry["pantry_item_id"]` being `None`.

This was a live bug, not a hypothetical: `eval/harness.py`'s deduction fixtures caught it directly — case 6 (`natural yogurt` recipe ingredient against a `greek yogurt 500g` pantry item, expected no-match) and case 8 (`spring onions` against `red onion 1`, expected no-match) were both coming back as confident-enough matches (confidence 0.7 and 0.35) to auto-populate a deduction plan the user would have had to catch and reject by hand.

### Architectural decisions
Based this branch on `main`, not on the currently-open `eval/hardening-cycle-1` (PR #27), even though that's where the deduction eval was last run — `pantry_deductor.py` is untouched by either open eval-harness PR, and stacking an unrelated production bugfix on top of two not-yet-merged PRs would make it wait on both of them to merge before it could ship, for no benefit.

Kept the confidence floor in code rather than relying solely on the new prompt guidance, deliberately redundant with it: the prompt rules cover the three lookalike pairs this bug report named, but the floor catches any other case Haiku is under-confident about, named or not, and doesn't depend on Haiku actually following the instruction — a repeat of this exact bug class with a different pair of ingredients (e.g. cilantro vs. parsley) would already be caught by the floor even without a fourth named prompt rule for it.

Verified the floor doesn't just move the bug into `apply_deduction`: a `match_found: False` entry keeps its `pantry_item_id` as `None` per the fix's own spec (for PM visibility), so `apply_deduction` unconditionally trying to look that row up in Supabase would raise. Added the one-line skip there rather than leaving it as a latent crash the next caller would discover.

Ran only `eval.harness.eval_deduction()` (not the full harness) three times in a row per the task, rather than once, specifically because prior harness runs on this codebase have shown Haiku's matching to be non-deterministic run-to-run — a single green run wouldn't have been enough evidence the fix is what's holding, as opposed to Haiku happening not to hit a lookalike-pair edge case that run.

### Bugs found and fixed
The false-positive matches described above (cases 6 and 8) are the bug this PR fixes. One additional issue surfaced while confirming the fix, not fixed here because it's in `eval/harness.py`, out of this PR's file scope: `eval_deduction`'s `actual_match = len(plan) > 0` check predates this fix, from back when a no-match ingredient was simply omitted from the plan. Now that a low-confidence match is deliberately *kept* in the plan (as a `match_found: False` entry, per this fix's own spec), a plan containing only such entries is non-empty, so `len(plan) > 0` reads as a match even though `match_found` says otherwise — reproduced directly: one of the three-in-a-row confirmation runs had Haiku return a `confidence: 0.0` guess on case 13 (parmesan cheese vs. cheddar cheese, expected no-match), correctly zeroed out by the new floor to `match_found: False`, but still counted as `actual_match: True` by the harness's stale check, failing a case the fix actually handled correctly. `eval_deduction` should check `any(e.get("match_found") for e in plan)` instead of `len(plan) > 0`; flagged here for a follow-up PR rather than patched in this one since it touches `eval/harness.py`, not `pantry_deductor.py`.
